...
> 命名，是为了更规范，更标准，可读性高，可维护性高；但不一定非要遵循，而且每个公司都可以不一样，需要根据实际情况而定。

- **蛇形**（snake case），下划线将单词连接，比如：file_name
- **驼峰** 分为小驼峰（camelCase）和大驼峰（CamelCase）
- **后缀** BO、DTO、VO（Result）、Param、R\<T\>

![alibaba 规范](../public/images/Java/alibaba-name.png)
