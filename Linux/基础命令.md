...
# 2. yum在线安装
yum -y install xxx # yum install 安装命令: -y所有的提示都为y，--installroot指定安装路径，--downloadonly只下载，不安装，list查看所有安装列表，search搜索关键字对应的包
yum -y update 包名							 # 升级
...
yum list installed | grep "nginx"   # 如果是使用yum安装，那就自己找办法
